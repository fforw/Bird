...
// http://the.fuchsia-design.com/2006/12/getting-svg-elementss-full-bounding-box.html
    ...
    getScreenBBox_impl= function (element) {

        // macro to create an SVGPoint object
        function createPoint (x, y) {

            var point = findSvg(element).createSVGPoint();
            point.x = x;
            point.y = y;
            return point;
        }
        ...
        function createRect (x, y, width, height) {
            var rect = findSvg(element).createSVGRect();
            rect.x = x;
            rect.y = y;
            rect.width = width;
            rect.height = height;
...
window.getScreenBBox = getScreenBBox;

module.exports = getScreenBBox;
